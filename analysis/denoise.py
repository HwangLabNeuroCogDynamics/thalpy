...
def denoise(
    ...
    if col_names:
        df = df.append(df_orig[col_names])
    ...
    if hp_filter:
        ...
        assert hp_filter > 0
        df = make_first_level_design_matrix(
            frame_times,
            high_pass=hp_filter,
            add_regs=df.values,
    ...
    if lp_filter:
        ...
        ## temp img for html report
        # temp_img_file = pjoin(out_path, file_base + "_temp" + nii_ext)
        # temp_img = nb.Nifti1Image(
        #     np.reshape(
        #         results.residuals.T + np.reshape(data_mean, (Nvox, 1)), img.shape
        #     ).astype("float32"),
        #     img.affine,
        #     header=img.header,
        # )
